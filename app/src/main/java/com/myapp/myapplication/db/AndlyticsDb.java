...
import com.myapp.myapplication.Preferences;
import com.myapp.myapplication.model.AppDetails;
import com.myapp.myapplication.model.AppInfo;
import com.myapp.myapplication.model.DeveloperAccount;
import com.myapp.myapplication.model.Link;
...
public class AndlyticsDb extends SQLiteOpenHelper {
	...
	public synchronized void fetchAppDetails(AppInfo appInfo) {
		...
		try {
			...
			@SuppressLint("Range") Long id = c.getLong(c.getColumnIndex(AppDetailsTable.ROWID));
			@SuppressLint("Range") String description = c.getString(c.getColumnIndex(AppDetailsTable.DESCRIPTION));
	...
	public synchronized void fetchRevenueSummary(AppInfo appInfo) {
		...
		try {
			...
			@SuppressLint("Range") Long id = c.getLong(c.getColumnIndex(RevenueSummaryTable.ROWID));
			@SuppressLint("Range") int typeIdx = c.getInt(c.getColumnIndex(RevenueSummaryTable.TYPE));
			@SuppressLint("Range") String currency = c.getString(c.getColumnIndex(RevenueSummaryTable.CURRENCY));
			...
			@SuppressLint("Range") double lastDayTotal = c.getDouble(c.getColumnIndex(RevenueSummaryTable.LAST_DAY_TOTAL));
			@SuppressLint("Range") double last7DaysTotal = c.getDouble(c
					.getColumnIndex(RevenueSummaryTable.LAST_7DAYS_TOTAL));
			@SuppressLint("Range") double last30DaysTotal = c.getDouble(c
					.getColumnIndex(RevenueSummaryTable.LAST_30DAYS_TOTAL));
			@SuppressLint("Range") double overallTotal = c.getDouble(c.getColumnIndex(RevenueSummaryTable.OVERALL_TOTAL));
