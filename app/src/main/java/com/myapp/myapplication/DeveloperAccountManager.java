...
import com.myapp.myapplication.db.AndlyticsDb;
import com.myapp.myapplication.db.DeveloperAccountsTable;
...
public class DeveloperAccountManager {
	...
	private DeveloperAccount createAcount(Cursor c) {
		@SuppressLint("Range") long id = c.getLong(c.getColumnIndex(DeveloperAccountsTable.ROWID));
		@SuppressLint("Range") String name = c.getString(c.getColumnIndex(DeveloperAccountsTable.NAME));
		@SuppressLint("Range") DeveloperAccount.State currentState = DeveloperAccount.State.values()[c.getInt(c
				.getColumnIndex(DeveloperAccountsTable.STATE))];
		@SuppressLint("Range") Date lastStatsUpdate = new Date(c.getLong(c
