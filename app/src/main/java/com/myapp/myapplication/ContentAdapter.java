...
import com.myapp.myapplication.Preferences.Timeframe;
import com.myapp.myapplication.db.AdmobTable;
import com.myapp.myapplication.db.AppInfoTable;
import com.myapp.myapplication.db.AppStatsTable;
import com.myapp.myapplication.db.CommentsTable;
import com.myapp.myapplication.db.RevenueSummaryTable;
import com.myapp.myapplication.model.AdmobStats;
import com.myapp.myapplication.model.AdmobStatsSummary;
import com.myapp.myapplication.model.AppInfo;
import com.myapp.myapplication.model.AppStats;
import com.myapp.myapplication.model.AppStatsSummary;
import com.myapp.myapplication.model.Comment;
import com.myapp.myapplication.model.Revenue;
import com.myapp.myapplication.model.RevenueSummary;
import com.myapp.myapplication.util.Utils;
...
public class ContentAdapter {
		...
		if (revenue != null) {
			...
			try {
				...
				if (!c.moveToNext()) {
					...
					@SuppressLint("Range") long revenueId = c.getLong(c.getColumnIndex(RevenueSummaryTable.ROWID));
	...
	public RevenueSummary getRevenueSummaryForApp(AppInfo app) {
		...
		try {
			...
			@SuppressLint("Range") int typeIdx = cursor.getInt(cursor.getColumnIndex(RevenueSummaryTable.TYPE));
			@SuppressLint("Range") String currency = cursor.getString(cursor.getColumnIndex(RevenueSummaryTable.CURRENCY));
			...
			@SuppressLint("Range") double lastDay = cursor.getDouble(cursor
					.getColumnIndex(RevenueSummaryTable.LAST_DAY_TOTAL));
			@SuppressLint("Range")double last7Days = cursor.getDouble(cursor
					.getColumnIndex(RevenueSummaryTable.LAST_7DAYS_TOTAL));
			@SuppressLint("Range")double last30Days = cursor.getDouble(cursor
					.getColumnIndex(RevenueSummaryTable.LAST_30DAYS_TOTAL));
			@SuppressLint("Range")double overall = cursor.getDouble(cursor
